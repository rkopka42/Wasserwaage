  ...
  for (n=0;n<p.length();n++)
  {
    switch (state)
    {
      case SEARCH:
        ...
        break;
      case SEARCHCMD:
        if (p.substring(n,n+2) == "QQ")
        {
          state = READY;
          Serial.println("Fertig");
          n++;   // egal Rest wird ignoriert
          break; 
        }
...
unsigned long f_min=86800, f_max=42900, f_delta=f_min - f_max;  // 43900
float factor[10] = {  1.0,0.94,0.82,0.64,0.47,0.31,0.19,0.12,0.05,0.00  };
//                    0l   10l                                      90l

// Liter zurück 0-90  0.1 und 90.1 als Endmarken
float getliter(unsigned long count)
{
  float liter=0;
  int n;
  float f = (float)(count-f_max);   // 67446 -> 19354
  f /= f_delta;                       // 0.44
  
  //Serial.print(" f=" + String(f) + " ");

  if (count>=f_min)  return 0.1;
  if (count<=f_max)  return 90.1;
  for (n=0;n<10;n++)
    if (f>factor[n])
    {
      //Serial.print(" n=" + String(n) + " factor[n]=" + String(factor[n]) + " ");
      // Wert liegt zwischen n und n-1
      float d = factor[n-1] - factor[n];  // der Prozentabstand in dem Feld
      float dd = (f - factor[n]) / d;   // Prozentsatz in dem Feld
      liter = (n - dd) * 10;
      //Serial.print(" d=" + String(d) + " dd=" + String(dd) + " ");
      break;
    }
  return liter;
}
